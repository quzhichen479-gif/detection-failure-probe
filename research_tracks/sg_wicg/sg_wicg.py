"""SG-WICG v1 reference implementation for YOLO11 / Ultralytics 8.4.113.

This file implements only A1-A5 foreground box-regression geometry. A0 must keep
Ultralytics' native bbox_iou(..., CIoU=True) path. TAL, DFL, cls loss, Detect,
decoding and NMS are outside the scope of this module.

The first six-cell ablation is intentionally single-GPU for A5. Wise population
statistics are therefore local here. See README.md before adding DDP support.
"""
...
class SGWICGConfig:
    """Frozen v1 defaults; do not tune during the first six-cell ablation."""
...
VALID_MODES = {"inner_ciou", "gcd", "inner_gcd_fixed", "sg_icg", "sg_wicg"}
...
def aligned_iou(pred: Tensor, target: Tensor, eps: float = 1e-7) -> Tensor:
    """Aligned IoU for N xyxy prediction/target pairs."""
    ...
    union = (p_wh[:, 0] * p_wh[:, 1] + t_wh[:, 0] * t_wh[:, 1] - inter).clamp_min(eps)
...
def inner_ciou_loss(pred: Tensor, target: Tensor, ratio: float = 1.25, eps: float = 1e-7) -> Tensor:
    """L = 1-IoU(inner) + center-distance penalty + CIoU aspect penalty."""
    if ratio <= 0:
        raise ValueError(f"inner_ratio must be > 0, got {ratio}")

    inner_iou = aligned_iou(_scale_about_center(pred, ratio), _scale_about_center(target, ratio), eps)
    iou = aligned_iou(pred, target, eps)
...
def gcd_loss(pred: Tensor, target: Tensor, eps: float = 1e-6) -> Tensor:
    ...
    similarity = torch.exp(-torch.sqrt(gcd2.clamp_min(0.0)))
...
def scale_gate_from_target(
    ...
    """GT-short-side gate in current network-input pixels."""
    if temp_px <= 0:
    ...
    short_px = (wh_grid * stride_pos[:, None]).amin(dim=-1)
...
class WiseFocus(nn.Module):
    ...
    def __init__(
        ...
        self.alpha, self.delta, self.ema_rate, self.eps = float(alpha), float(delta), float(ema_rate), float(eps)
        self.register_buffer("iou_mean", torch.tensor(float(mean_init), dtype=torch.float32))

    @torch.no_grad()
    def update_mean(self, q: Tensor) -> None:
        if q.numel():
            self.iou_mean.mul_(1.0 - self.ema_rate).add_(self.ema_rate * q.detach().float().mean())
    ...
    def forward(self, plain_iou: Tensor, tal_weight: Tensor, update_state: bool = True) -> Tuple[Tensor, Dict[str, Tensor]]:
        ...
        mean = self.iou_mean.to(q).clamp_min(self.eps)
        beta = q / mean
        alpha = torch.as_tensor(self.alpha, device=q.device, dtype=q.dtype)
        raw = beta / (self.delta * torch.pow(alpha, beta - self.delta)).clamp_min(self.eps)

        w = tal_weight.detach().reshape(-1).to(raw.dtype)
        weighted_raw_mean = (raw * w).sum() / w.sum().clamp_min(self.eps)
        gain = (raw / weighted_raw_mean.clamp_min(self.eps)).detach()
        weighted_gain_mean = (gain * w).sum() / w.sum().clamp_min(self.eps)
        return gain, {
            "wise_iou_mean": mean.detach(),
            "wise_beta_mean": beta.detach().mean(),
            "wise_gain_mean": weighted_gain_mean.detach(),
            "wise_gain_max": gain.detach().max(),
        }


class SGWICGBoxLoss(nn.Module):
    """Foreground-only A1-A5 box regression loss with Ultralytics normalization."""

    def __init__(self, cfg: SGWICGConfig | None = None) -> None:
        super().__init__()
        self.cfg = cfg or SGWICGConfig()
        if self.cfg.mode not in VALID_MODES:
            raise ValueError(f"unsupported mode {self.cfg.mode}; valid={sorted(VALID_MODES)}")
        self.wise = WiseFocus(
            self.cfg.wise_alpha,
            self.cfg.wise_delta,
            self.cfg.wise_ema_rate,
            self.cfg.wise_mean_init,
            self.cfg.eps,
        )
    ...
    def forward(
        ...
        *,
        ...
        if pred_xyxy_grid.numel() == 0:
            return pred_xyxy_grid.sum() * 0.0, {}

        w = tal_weight.reshape(-1).to(pred_xyxy_grid.dtype)
        ...
        if mode == "inner_ciou":
            ...
            lam_fixed = float(self.cfg.gcd_fixed_weight)
            if not 0.0 <= lam_fixed <= 1.0:
                raise ValueError("gcd_fixed_weight must be in [0,1]")
            per_pos = (1.0 - lam_fixed) * l_inner + lam_fixed * l_gcd
        else:
            lam, short_px = scale_gate_from_target(target_xyxy_grid, stride_pos, self.cfg.sg_tau_px, self.cfg.sg_temp_px)
            ...
            if mode == "sg_wicg":
                ...
                diag.update(wise_diag)

        loss = (per_pos * w).sum() / denom
        diag["box_reg_loss"] = loss.detach()
        return loss, diag


def foreground_strides(stride_tensor: Tensor, fg_mask: Tensor) -> Tensor:
    """Extract one stride scalar per foreground sample from 8.4.113 tensors."""
    ...
    return stride_tensor.unsqueeze(0).expand(fg_mask.shape[0], -1, -1)[fg_mask].reshape(-1)
